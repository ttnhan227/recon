...
async def run_doctor(target_url: str | None = None) -> bool:
    ...
    if sys.version_info >= (3, 12):
        ...
        table.add_row("Python Version", "[yellow]WARN[/yellow]", f"Python {py_ver} (3.12+ recommended)")
    ...
    if provider_name == "gemini":
        ...
        table.add_row("AI Provider (Gemini)", status_str, f"Key present: {has_key} (Model: {settings.gemini_model})")
    elif provider_name == "mistral":
        has_key = bool(settings.mistral_api_key)
        status_str = "[green]CONFIGURED[/green]" if has_key else "[yellow]NO KEY[/yellow]"
        table.add_row("AI Provider (Mistral)", status_str, f"Key present: {has_key} (Model: {settings.mistral_model})")
    elif provider_name == "openai":
        has_key = bool(settings.openai_api_key)
        status_str = "[green]CONFIGURED[/green]" if has_key else "[yellow]NO KEY[/yellow]"
        table.add_row("AI Provider (OpenAI)", status_str, f"Key present: {has_key} (Model: {settings.openai_model})")
    else:
        table.add_row("AI Provider (Mock)", "[green]MOCK READY[/green]", "Deterministic offline LLM simulation")
    ...
    if target_url:
        try:
            async with httpx.AsyncClient(timeout=4.0, verify=False) as client:
                ...
                table.add_row("Target Reachability", "[green]OK[/green]", f"{target_url} returned HTTP {resp.status_code}")
        except Exception as e:
            all_healthy = False
            table.add_row("Target Reachability", "[red]FAIL[/red]", f"Cannot connect to {target_url}: {e}")
