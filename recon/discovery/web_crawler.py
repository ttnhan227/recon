...
class WebCrawler:
    ...
    async def crawl(self) -> DiscoveredApplication:
        ...
        try:
            ...
            logger.warning(
                f"Playwright web crawl unavailable ({e}), falling back to static HTML crawler."
            )
    ...
    async def _crawl_with_playwright(self) -> DiscoveredApplication:
        ...
        async with async_playwright() as p:
            ...
            while queue and len(self.discovered_pages) < self.max_pages:
                ...
                page.on(
                    "pageerror", lambda exc: console_errors.append(f"Uncaught Exception: {exc}")
                )
                page.on(
                    "requestfailed",
    ...
    async def _parse_playwright_page(
        ...
        for form_idx, f_handle in enumerate(await page.query_selector_all("form")):
            try:
                ...
                for inp_handle in await f_handle.query_selector_all("input, select, textarea"):
                    try:
                        name = (
                            await inp_handle.get_attribute("name")
                            or await inp_handle.get_attribute("id")
                            or ""
                        ).strip()
                        if not name:
                            continue
                        ...
                        input_type = (
                            (await inp_handle.get_attribute("type") or "text")
                            if tag == "input"
                            else tag
                        )
        ...
        for btn_idx, b_handle in enumerate(
            await page.query_selector_all("button, [role='button']")
        ):
            try:
    ...
    def _parse_html_content(
        ...
        def _str_attr(el: Any, attr: str, default: str = "") -> str:
            v = el.get(attr)
            if v is None:
                return default
        ...
        def _optional_str_attr(el: Any, attr: str) -> str | None:
            v = el.get(attr)
            if v is None:
                return None
            if isinstance(v, (list, tuple)):
        ...
        for form_idx, form in enumerate(soup.find_all("form")):
            form_action = urljoin(url, _str_attr(form, "action", ""))
            form_method = _str_attr(form, "method", "GET").upper()
            form_id = _optional_str_attr(form, "id")
            form_selector = f"#{form_id}" if form_id else f"form:nth-of-type({form_idx + 1})"

            fields: list[DiscoveredFormField] = []
            for inp in form.find_all(["input", "select", "textarea"]):
                name = _str_attr(inp, "name") or _str_attr(inp, "id")
                if not name:
                    continue
                tag = inp.name
                input_type = _str_attr(inp, "type", "text") if tag == "input" else tag
                required = inp.has_attr("required")
                placeholder = _optional_str_attr(inp, "placeholder")
                default_val = _optional_str_attr(inp, "value")

                options: list[str] = []
                if tag == "select":
                    for opt in inp.find_all("option"):
                        val = _optional_str_attr(opt, "value") or opt.text
                        if val:
                            options.append(val.strip())

                inp_id = _optional_str_attr(inp, "id")
                selector = f"#{inp_id}" if inp_id else f"[name='{name}']"
            ...
            if submit_btn:
                btn_id = _optional_str_attr(submit_btn, "id")
                submit_selector = (
                    f"#{btn_id}"
                    if btn_id
                    else f"{form_selector} button[type='submit'], {form_selector} input[type='submit']"
                )
